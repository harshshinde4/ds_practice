...
/* In stdlib.h -->
        #ifdef _CRT_RAND_S
        error_t rand_s(unsigned int* p_num);
        #endif
*/
...
/*
    Routines that are called by the clients to make
    use of the list
*/
...
int reversed_list(struct node* p_list);
...
int main(void)
{
    ...
    if(find_data(p_list, 200) == TRUE)
        puts("200 is present in the list at the moment");
    
    if(find_data(p_list, -5214) == FALSE)
        puts("-2514 is not present in the liat at the moment");
...
struct node* create_list(void)
{
    ...
    p_list = get_new_node(-1); // why -1? -> Parent node without any data
    return (p_list);
...
void extreme_testing(void)
{
    unsigned long long int nr_elements = 5000; 
